...
@vite(["resources/js/admin.js"])
    ...
    <div class="w-full mx-12 flex flex-col items-center ">
        ...
        <form action="{{ route('admin.words.marc.update') }}" method="POST" class="w-full">
            @csrf
            <input type="hidden" name="is_primary" value="1">
            <input type="hidden" name="author" value="Marc Belderbos">
            <input type="hidden" name="slug" value="words-by-marc">
            ...
            <div id="accordion-collapse" class="w-full" data-accordion="collapse">
                ...
                @if ($primary->isEmpty())
                    ...
                    <div id="accordion-collapse-body-fr" class="hidden w-full"
                        ...
                        <input type="hidden" name="fr[title]" value="Words by Marc">
                    ...
                    <div id="accordion-collapse-body-en" class="hidden" aria-labelledby="accordion-collapse-heading-en">
                        <input type="hidden" name="en[title]" value="Words by Marc">
                    ...
                    <div id="accordion-collapse-body-nl" class="hidden" aria-labelledby="accordion-collapse-heading-nl">
                        <input type="hidden" name="nl[title]" value="Words by Marc">
                ...
                @foreach ($primary as $article)
                    ...
                    <div id="accordion-collapse-body-{{ $article->language }}" class="hidden"
                        ...
                        <input type="hidden" name="{{ $article->language }}[title]" value="Words by Marc">
        ...
        <form enctype="multipart/form-data" method="POST" action="{{ route('admin.words.marc.store') }}"
        ...
        <div class="w-full flex flex-wrap gap-7 justify-around bg-slate-100 p-12 mb-12 rounded-md">
            ...
            <form id="words-list" action="{{route("admin.words.marc.update-order")}}" method="post" id="articles-list" class="w-full flex flex-wrap gap-7 justify-around bg-slate-100 p-12 mb-12 rounded-md">
                @csrf   
                @foreach ($articles as $article)
                <div id="article-item-{{$article->id}}" draggable="true" class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <input type="hidden" name="articles[]" value="{{$article->slug}}">
                    <div class="flex justify-end px-4 pt-4">
                        <button id="{{$article->slug}}" data-dropdown-toggle="dropdown-{{$article->id}}" class="inline-block text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-1.5" type="button">
                        ...
                        <!-- Dropdown menu -->
                        <div id="dropdown-{{$article->id}}" class="z-10 hidden text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                            <ul class="py-2" aria-labelledby="{{$article->slug}}">
                            <li>
                                <a href="{{route("admin.words.marc.editDetail", ["Word" => $article->slug])}}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Edit</a>
                            </li>
                            <li>
                    ...
                    <div class="cursor-move flex flex-col items-center pb-10">
                        <img class="h-36 w-auto mb-3 rounded-md shadow-lg" src="{{asset($article->cover)}}" alt="Bonnie image"/>
