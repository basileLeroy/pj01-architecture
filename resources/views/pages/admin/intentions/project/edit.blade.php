...
<div class="w-full mx-12 flex flex-col items-center ">
    <h1 class="text-3xl font-bold my-12">Intensions lors d'un projet</h1>

    <form action="{{ route('admin.intentions-project.update')}}" method="POST" class="w-full">
        @csrf
        ...
        <div class="flex mt-20 w-full justify-center gap-16">
            ...
            <a href="{{ route("admin.intentions-project.edit")}}" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:focus:ring-yellow-900">Cancel</a>
        </div>
    </form>
    @if ($errors->any())
