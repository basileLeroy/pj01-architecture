...
@section('seo')
    belderbos, marc, architecturer, products, books, architecture
@endsection
...
@section('content')
    ...
    <div id="page" class="w-full mx-12 flex flex-col items-center ">
        ...
        <form enctype="multipart/form-data" method="POST" action="{{ route('admin.products.store') }}"
            ...
            @csrf
            <h2 class="text-2xl font-bold">Ajouter une nouvelle édition</h2>

            <div class="my-5">
                <label for="title-input" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Titre
                    *</label>
            ...
            <div class="mb-5">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="link">
                    Lien pour acheter le livre (par example: un lien Amazon)
                </label>
                <input
                    class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                ...
                <div id="project-item" draggable="true"
                    ...
                    <div class="flex justify-end px-4 pt-4">
                        ...
                        <div id="dropdown-{{ $product->id }}"
                            ...
                            <ul class="py-2" aria-labelledby="{{ $product->slug }}">
                                <li>
                    ...
                    <div class="cursor-move flex flex-col items-center pb-10">
                        <img class="h-36 w-auto mb-3 rounded-md shadow-lg" src="{{ asset($product->cover) }}"
                            alt="Bonnie image" />
                        <h5 class="mb-1 text-center mx-6 text-xl font-medium text-gray-900 dark:text-white">
                            {{ $product->title }}</h5>
                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ $product->updated_at }}</span>
                    </div>
                </div>
            @empty
                <p>Pas d'éditions pour le moment.</p>
            @endforelse
            <div class="flex mt-20 w-full justify-center gap-16">
                <button type="submit"
                    class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-md px-14 py-2.5 me-2 mb-2  dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Save</button>
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <a data-modal-hide="popup-modal"
                        href="{{ route('admin.products.delete', ['Product' => $product->slug]) }}"
